...
export CLANG_PATH=~/kernel/dragontc/bin/clang
...
if [ "$choice" == "1" ]; then
  ...
  make CC=~/kernel/dragontc/bin/clang $CONFIG $THREAD &>/dev/null
  make CC=~/kernel/dragontc/bin/clang $THREAD &>buildlog.txt & pid=$!
  spin[0]="$blue-"
  spin[1]="\\"
  spin[2]="|"
  spin[3]="/$nc"
